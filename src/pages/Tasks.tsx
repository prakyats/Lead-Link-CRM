...
import { getTasks, createTask as createTaskApi, toggleCompleteTask, getTaskSummary } from '@/api/tasks';
...
export default function Tasks() {
  ...
  const [timeTab, setTimeTab] = useState<'all' | 'overdue' | 'today' | 'upcoming'>('all');
  ...
  const timeFilteredTasks = useMemo(() => {
    ...
    const base = filteredTasks as any[];

    // For SALES, use API buckets for time tabs (as requested)
    if (user?.role === 'SALES' && timeTab !== 'all') {
      const summary = taskSummaryQuery.data as any;
      if (!summary) return [];
      if (timeTab === 'overdue') return summary.overdue || [];
      if (timeTab === 'today') return summary.today || [];
      if (timeTab === 'upcoming') return summary.upcoming || [];
      return base;
    }

    // For MANAGER, derive time buckets from current list (pending only)
    if (user?.role === 'MANAGER' && timeTab !== 'all') {
      const pending = base.filter(t => t.status === 'PENDING');
      if (timeTab === 'overdue') return pending.filter(t => new Date(t.dueDate) < now);
      if (timeTab === 'today') return pending.filter(t => {
        const d = new Date(t.dueDate);
        return d >= start && d <= end;
      });
      if (timeTab === 'upcoming') return pending.filter(t => new Date(t.dueDate) > end);
    }

    return base;
  }, [filteredTasks, timeTab, taskSummaryQuery.data, user?.role]);

  const groupedTasksForManager = useMemo(() => {
    if (user?.role !== 'MANAGER') return null;
    const map = new Map<string, any[]>();
    for (const t of timeFilteredTasks as any[]) {
      const key = (t.assignedTo || 'Unassigned').toString();
  ...
  const overdueCount = useMemo(() => (tasks as any[]).filter(t => t.status === 'PENDING' && isOverdue(t.dueDate)).length, [tasks]);
  const todayCount = useMemo(() => {
    const now = new Date();
    const start = new Date(now); start.setHours(0, 0, 0, 0);
    const end = new Date(now); end.setHours(23, 59, 59, 999);
    return (tasks as any[]).filter(t => t.status === 'PENDING' && new Date(t.dueDate) >= start && new Date(t.dueDate) <= end).length;
  }, [tasks]);
  const upcomingCount = useMemo(() => {
    const now = new Date();
    const end = new Date(now); end.setHours(23, 59, 59, 999);
    return (tasks as any[]).filter(t => t.status === 'PENDING' && new Date(t.dueDate) > end).length;
  }, [tasks]);

  const salesCounts = useMemo(() => {
    if (user?.role !== 'SALES') return null;
    const s = taskSummaryQuery.data as any;
    if (!s) return null;
    return {
      overdue: (s.overdue || []).length,
      today: (s.today || []).length,
      upcoming: (s.upcoming || []).length,
      all: ((s.overdue || []).length + (s.today || []).length + (s.upcoming || []).length)
    };
  }, [taskSummaryQuery.data, user?.role]);
      ...
      <main className="crm-main-content">
        ...
        <div className="relative z-10 p-10 space-y-10 custom-scrollbar overflow-y-auto h-full">
          ...
          <div className="crm-card !p-0 overflow-hidden bg-card/60 backdrop-blur-md">
            ...
            <div className="p-6">
              {(user?.role === 'MANAGER' || user?.role === 'SALES') && filter !== 'COMPLETED' && (
                <div className="mb-5 flex flex-wrap gap-2">
                  {[
                    {
                      id: 'all' as const,
                      label: 'All',
                      count: user?.role === 'SALES' ? (salesCounts?.all ?? pendingCount) : pendingCount
                    },
                    {
                      id: 'overdue' as const,
                      label: 'Overdue',
                      count: user?.role === 'SALES' ? (salesCounts?.overdue ?? overdueCount) : overdueCount
                    },
                    {
                      id: 'today' as const,
                      label: 'Today',
                      count: user?.role === 'SALES' ? (salesCounts?.today ?? todayCount) : todayCount
                    },
                    {
                      id: 'upcoming' as const,
                      label: 'Upcoming',
                      count: user?.role === 'SALES' ? (salesCounts?.upcoming ?? upcomingCount) : upcomingCount
                    },
                  ].map((t) => (
                    <button
                      key={t.id}
                      ...
                      className={`px-4 py-2 rounded-xl text-[10px] font-bold uppercase tracking-widest border transition-all ${
                        timeTab === t.id
                          ? (t.id === 'overdue' ? 'bg-status-danger/10 border-status-danger/20 text-status-danger' : 'bg-primary/10 border-primary/20 text-primary')
                          : 'bg-muted/10 border-border/40 text-muted-foreground/60 hover:text-foreground hover:bg-muted/20'
                      }`}
                    >
                      {t.label} ({t.count})
                    </button>
              ...
              <div className="space-y-3">
                {loading ? (
                   <TaskSkeleton />
                ) : user?.role === 'MANAGER' && groupedTasksForManager ? (
                  <Accordion type="multiple" className="rounded-2xl border border-border/40 overflow-hidden">
                    {groupedTasksForManager.map((group) => {
                      const overdue = group.items.filter((t: any) => t.status !== 'COMPLETED' && isOverdue(t.dueDate)).length;
                      const pending = group.items.filter((t: any) => t.status === 'PENDING').length;
                      return (
                        <AccordionItem key={group.assignee} value={group.assignee} className="border-border/40">
                          <AccordionTrigger className="px-6 py-5 hover:no-underline">
                            <div className="flex items-center justify-between w-full gap-4">
                              <div className="min-w-0">
                                <div className="flex items-center gap-3">
                                  <div className="w-10 h-10 rounded-2xl bg-primary/10 border border-primary/20 flex items-center justify-center text-primary font-bold text-sm shadow-inner">
                                    {(group?.assignee?.trim?.()?.[0] ?? '?').toUpperCase()}
                                  </div>
                                  <div className="min-w-0">
                                    <div className="text-sm font-bold text-foreground truncate">{group.assignee}</div>
                                    <div className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground/60 mt-1">
                                      {group.items.length} tasks • {pending} pending • {overdue} overdue
                                    </div>
                                  </div>
                                </div>
                              </div>
                              {overdue > 0 && (
                                <div className="px-3 py-1.5 rounded-xl text-[10px] font-bold uppercase tracking-widest bg-status-danger/10 text-status-danger border border-status-danger/20 shrink-0">
                                  {overdue} critical
                                </div>
                              )}
                            </div>
                ...
                ) : (timeFilteredTasks as any[]).map((task) => {
                  const taskOverdue = task.status !== 'COMPLETED' && isOverdue(task.dueDate);
                  return (
                    <motion.div
                      layout
                ...
                {!loading && ((user?.role === 'MANAGER' ? groupedTasksForManager?.length === 0 : (timeFilteredTasks as any[])?.length === 0)) && (
