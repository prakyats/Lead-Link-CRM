...
export default function TeamInsights() {
  ...
  const aggregateStats = useMemo(() => {
    ...
    const totalLeads = performance.reduce((acc: number, curr: any) => acc + curr.totalLeads, 0);
    const totalConverted = performance.reduce((acc: number, curr: any) => acc + curr.convertedLeads, 0);
    const avgConversion = totalLeads > 0 ? ((totalConverted / totalLeads) * 100).toFixed(1) : 0;
    const totalTasksCompleted = performance.reduce((acc: number, curr: any) => acc + curr.tasksCompleted, 0);
    const totalTasksPending = performance.reduce((acc: number, curr: any) => acc + curr.tasksPending, 0);
          ...
          <div className="max-w-7xl mx-auto space-y-10">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
              ...
              <div className="lg:col-span-2 space-y-6">
                <div className="crm-card bg-card/20 backdrop-blur-3xl border-white/5 !p-0 overflow-hidden">
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-left">
                      ...
                      <tbody className="divide-y divide-border/20">
                        {performance?.map((agent: any, i: number) => (
                          <motion.tr 
                            key={agent.id}
                          ...
                          >
                            <td className="px-8 py-5">
                              <div className="flex items-center gap-3">
                                <div className="w-9 h-9 rounded-xl bg-card border border-border/40 flex items-center justify-center text-[10px] font-black text-primary group-hover:border-primary/40 transition-all uppercase">
                                  {agent.name.split(' ').map((n: string) => n[0]).join('')}
                                </div>
                                <span className="text-sm font-bold text-foreground group-hover:text-primary transition-colors">{agent.name}</span>
                              </div>
                            </td>
                            <td className="px-8 py-5 text-center text-xs font-bold tabular-nums">{agent.totalLeads}</td>
                            <td className="px-8 py-5 text-center">
                              <span className={`text-xs font-bold tabular-nums ${agent.conversionRate > 15 ? 'text-emerald-400' : 'text-foreground/60'}`}>
                    ...
                    {pipelineStages.map((stage) => (
                      <div key={stage.key} className="space-y-2">
                        <div className="flex justify-between items-end">
                          ...
                          <span className="text-xs font-bold tabular-nums">{pipeline[stage.key] || 0}</span>
                        </div>
                        <div className="h-2 w-full bg-white/5 rounded-full overflow-hidden">
                          <motion.div 
                            initial={{ width: 0 }}
                            animate={{ width: `${((pipeline[stage.key] || 0) / maxStageCount) * 100}%` }}
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center gap-4 p-4 rounded-2xl bg-rose-500/10 border border-rose-500/20">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] font-semibold text-rose-400/60 uppercase tracking-widest mt-0.5">{risk?.overdueTasksCount} overdue follow-ups</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-4 p-4 rounded-2xl bg-amber-500/10 border border-amber-500/20">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] font-semibold text-amber-400/60 uppercase tracking-widest mt-0.5">{risk?.inactiveLeadsCount} cold leads (>7d)</p>
                      </div>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="space-y-8 relative before:absolute before:left-[17px] before:top-2 before:bottom-2 before:w-px before:bg-white/5">
                    {activity?.map((act: any, i: number) => (
                      <div key={act.id} className="relative flex items-start gap-4">
                        <div className="w-9 h-9 rounded-xl border border-white/5 bg-muted flex items-center justify-center relative z-10 shrink-0">
                          <Activity size={14} className="text-primary/60" />
                        </div>
                        <div className="min-w-0 flex-1">
                          <p className="text-xs text-foreground/80 leading-snug">
                            <span className="font-bold text-primary">{act.performedBy.name}</span>
                            {' recorded a '}
                          ...
                          <p className="text-[10px] font-bold text-muted-foreground/30 uppercase tracking-widest mt-1 flex items-center gap-2">
                             {act.lead?.company || 'Lead'} • {new Date(act.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                          </p>
                        </div>
                      </div>
                    ))}
